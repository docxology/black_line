...
project. Version authority is `manuscript/config.yaml`; it is deliberately not
restated here.
...
- **Version authority** is `manuscript/config.yaml`; `pyproject.toml`,
...
- **Figures are inert unless embedded:** any new figure must be generated by
  ...
  referenced by filename in `manuscript/*.md`, with `interpretive_claim` and
  ...
  `manuscript/config.yaml`'s `paper.cover.image`, which `figures/validate.py`
...
- **In-figure text has a rendered floor:** `black_line.figures.legibility`
  ...
  `manuscript/config.yaml`; `tests/test_legibility.py` fails any figure whose
